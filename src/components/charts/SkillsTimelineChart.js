...
const SkillsTimelineChart = () => {
  ...
  const availableSkills = Object.keys(skillsGrowthData[0]).filter(
    key => key !== 'year'
  );
  ...
  return (
    <div className="skills-timeline-container">
          ...
          <button
            ...
            style={
              selectedSkills.includes(skill)
                ? { backgroundColor: skillColors[skill] || '#8884d8' }
                : {}
            }
          >
            {skill}
      ...
      <div className="skills-timeline-info">
        <p>
          This chart shows my skill development over the years. Toggle the
          buttons above to show/hide specific skills.
        </p>
      </div>
    </div>
  );
